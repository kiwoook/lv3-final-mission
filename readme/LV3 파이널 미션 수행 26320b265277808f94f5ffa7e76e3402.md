...
- Docker를 활용해서 DB와 운영 서버를 분리해서 처리할 수 있습니다.
- 브랜치에 따라서 dev와 prod 운영 서버를 다르게 처리할 수 있도록 하였습니다.
    - 두 개의 인스턴스를 설정하는 것이 가용성 측면에서 안전하다고 보지만, 현재 테스트에서는 하나의 인스턴스에서 두 개의 Spring 서버가 동작할 수 있도록 추가하였습니다.
- 하나의 도메인에서 Nginx를 사용해서 리버스 프록싱을 통해 두 개의 Spring 서버를 실행할 수 있도록 구축하였습니다.
- Nginx를 활용해 HTTPS를 설정하였습니다.

---

아래는 구축하는 과정을 추가하였습니다.
...
하나의 인스턴스에서만 사용하므로 nginx에서 다음과 같은 내용을 추가했다.

/etc/nginx/sites-available/default파일에 추가했다.

![{5606AFF0-15CA-4C1E-BAB7-D2F7323B1807}.png](5606AFF0-15CA-4C1E-BAB7-D2F7323B1807.png)
...
jobs:
  test:
    ...
    - name: Comment test results on PR
      ...
      run: |
        echo "✅ Tests passed for commit ${{ github.sha }}"

```

먼저 ci 작업을 실행 한 후에 cd파일이 동작되도록 설정하였다.
